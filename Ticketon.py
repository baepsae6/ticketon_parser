...
class Ticketon():
    ...
    def get_by_city_category(self, city_name, category_name):
        ...
        for city, category in self.cities_categories_dict.items():
            if category_name in category and city_name in city:
                for category_event, info in category.items():
                    city_category_events[category_event] = info
    ...
    def get_by_category_date(self, date, category_name):
        ...
        for category_event, info in category_events.items():
            if (category_event['time'] != None and date in category_event['time']):
